{
  ...
  outputs =
    ...
    {
      packages = {
        default = self.packages.${system}.less-delay-nro;

        less-delay-nro = pkgs.runCommandLocal "less-delay-nro-${self.packages.${system}.less-delay.version}" { } ''
          mkdir -p $out/lib
          ${self.packages.${system}.linkle}/bin/linkle nro ${self.packages.${system}.less-delay}/lib/libless_delay.so $out/lib/libless_delay.nro
        '';
